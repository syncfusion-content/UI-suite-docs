...
# Content Security Policy (CSP) in Vue Rich Text Editor Component
...
The following example shows a recommended CSP configuration for a Vue application that hosts the Syncfusion Rich Text Editor. The CSP is typically set in `public/index.html` (Vue CLI) or `index.html` (Vite), or supplied as a response header from your hosting server.
...
| `script-src` | `'self'` | Allows scripts from the same origin, including Vue's runtime and the editor's JavaScript. |
