---
layout: post
title: Content Security Policy in Blazor Rich Text Editor | Syncfusion®
description: Learn how to configure Content Security Policy for the Syncfusion Blazor Rich Text Editor.
platform: rich-text-editor-sdk
control: RichTextEditor
documentation: ug
---

# Content Security Policy (CSP) in Blazor Rich Text Editor
...
The following example shows a recommended CSP configuration for a Blazor application that hosts the Syncfusion Rich Text Editor. In Blazor Server and Blazor WebAssembly applications, the CSP is typically supplied as a response header from the ASP.NET Core middleware or set via a meta tag in `Pages/_Host.cshtml` (Blazor Server) or `wwwroot/index.html` (Blazor WebAssembly).
...
### Using a meta tag in `wwwroot/index.html` (Blazor WebAssembly)
...
| `script-src` | `'self'` | Allows scripts from the same origin, including Blazor's runtime and the editor's JavaScript. |
...
| `connect-src` | `'self' ws: wss:` | Restricts network connections to the same origin (WebSocket support is helpful for Blazor Server's SignalR circuit). |
