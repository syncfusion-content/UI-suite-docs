...
description: Learn here all about Content Security Policy in Syncfusion React Rich text editor component of Syncfusion Essential JS 2 and more.
...
The following example shows a recommended CSP configuration for a React application that hosts the Syncfusion Rich Text Editor. The CSP can be added in the `public/index.html` file (Create React App and Vite) or served as a response header from your hosting server.

### Using a meta tag in `public/index.html`
...
| `script-src` | `'self'` | Allows scripts from the same origin, including React's runtime helpers. |
