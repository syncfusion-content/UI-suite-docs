...
To show or hide specific columns during Excel export in Gantt, use the [toolbarClick](https://help.syncfusion.com/gantt-sdk/javascript/gantt-chart/events#toolbarclick) event to check `args.item.id` and update the [columns.visible](https://ej2.syncfusion.com/javascript/documentation/api/grid/column#visible) property to **true** or **false** . After the export is complete, restore the original column visibility using the [excelExportComplete](https://help.syncfusion.com/gantt-sdk/javascript/gantt-chart/events#excelexportcomplete) event.
...
You can customize Gantt cells in exported Excel or CSV documents using the [excelQueryCellInfo](https://help.syncfusion.com/gantt-sdk/javascript/gantt-chart/events#excelquerycellinfo) event. This event is triggered for each cell during export, allowing formatting to be conditionally applied based on the cell's content.

In the example below, the background color is customized for the **Progress** column in the exported Excel or CSV file:{% tabs %}
{% highlight js tabtitle="index.js" %}
